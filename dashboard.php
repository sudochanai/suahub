...
try {
    // Query to get the total number of houses
    $queryTotalHouses = "SELECT COUNT(*) AS total_houses FROM house";
    $resultTotalHouses = pg_query($conn, $queryTotalHouses);
    if ($resultTotalHouses) {
        $totalHouses = pg_fetch_result($resultTotalHouses, 0, 'total_houses');
    }

    // Query to get the total number of inventory
    $queryTotalInventory = "SELECT COUNT(*) AS total_inventory FROM inventory";
    $resultTotalInventory = pg_query($conn, $queryTotalInventory);
    if ($resultTotalInventory) {
        $totalInventory = pg_fetch_result($resultTotalInventory, 0, 'total_inventory');
    }

    // // Query to get the total number of signed contracts
    // $queryTotalSignedContracts = "SELECT COUNT(*) AS total_signed_contracts FROM contracts WHERE signed = TRUE";
    // $resultTotalSignedContracts = pg_query($conn, $queryTotalSignedContracts);
    // if ($resultTotalSignedContracts) {
    //     $totalSignedContracts = pg_fetch_result($resultTotalSignedContracts, 0, 'total_signed_contracts');
    // }

    // // Query to get the total number of unsigned contracts
    // $queryTotalUnsignedContracts = "SELECT COUNT(*) AS total_unsigned_contracts FROM contracts WHERE signed = FALSE";
    // $resultTotalUnsignedContracts = pg_query($conn, $queryTotalUnsignedContracts);
    // if ($resultTotalUnsignedContracts) {
    //     $totalUnsignedContracts = pg_fetch_result($resultTotalUnsignedContracts, 0, 'total_unsigned_contracts');
    // }

    // Commit transaction
    pg_query($conn, "COMMIT");

    // Now you can use these variables to display the results
    
   
    // echo "Total Signed Contracts: " . $totalSignedContracts . "<br>";
    // echo "Total Unsigned Contracts: " . $totalUnsignedContracts . "<br>";
...
//Deleting modal for the houses in the main dashboard
      ...
      <div class="container-fluid">
        ...
        <div class="row">
          <div class="col-md-3 mb-3">
            <div class="card bg-primary text-white h-100">
              <div class="card-body py-5"><?php echo" $totalHouses " ?> HOUSES</div>
          ...
          <div class="col-md-3 mb-3">
            <div class="card bg-success text-white h-100">
              <div class="card-body py-5"><?php echo"$totalInventory ";?>INVENTORY</div>
              <div class="card-footer d-flex">
                View Details
                <span class="ms-auto">
                  <i class="bi bi-chevron-right"></i>
          ...
          <div class="col-md-3 mb-3">
            <div class="card bg-warning text-dark h-100">
              <div class="card-body py-5">REPAIRS</div>
              <div class="card-footer d-flex">
                View Details
                <span class="ms-auto">
                  <i class="bi bi-chevron-right"></i>
          ...
          <div class="col-md-3 mb-3">
            <div class="card bg-danger text-white h-100">
              <div class="card-body py-5">PENDING PAYMENTS</div>
              <div class="card-footer d-flex">
                View Details
                <span class="ms-auto">
                  <i class="bi bi-chevron-right"></i>
        ...
        <div class='col-md-12 mb-3'>
            ...
            <div class='card-header'>
               <h4> <span> <i class='bi bi-house me-2'> </i> </span> Vacant Houses</h4>
            </div>
            <div class='card-body'>
              ...
              <?php
                  ...
                  <tbody>
                    ...
                    while($row = pg_fetch_assoc($result)){
                        ...
                        echo "<td><button type='submit' class='btn btn-outline-danger'>Renting Here</button></td>";
                        if ($role == 1 || $role == 2){
                            echo "<td>
                            ...
                            <button type='button' class='btn btn-outline-primary editHouseBtn'
                             ...
                             data-housestatus='{$row['housestatus']}'>
